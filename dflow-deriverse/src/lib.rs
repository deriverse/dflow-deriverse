...
impl Amm for Deriverse {
    fn from_keyed_account(
        keyed_account: &dflow_amm_interface::KeyedAccount,
        _: &dflow_amm_interface::AmmContext,
    ) -> Result<Self>
    where
        Self: Sized,
    {
    ...
    fn update(&mut self, account_map: &dflow_amm_interface::AccountMap) -> Result<()> {
        let ContextAccounts {
            instr_header,
            a_token_state_acc,
            b_token_state_acc,
    ...
    fn quote(
        &self,
        quote_params: &dflow_amm_interface::QuoteParams,
    ) -> Result<dflow_amm_interface::Quote> {
        let Deriverse {
            instr_header,
            b_token_state,
            order_book,
    ...
    ) -> Result<dflow_amm_interface::SwapAndAccountMetas> {
        let Deriverse {
            instr_header,
            accounts_ctx,
            a_token_state,
